...
public class MybatisController {
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
			...
			[동작방식]
				컨트롤러에서 메서드 호출
				-> inserface의 추상메서드 호출
				-> namespace에 해당 interface를 namespace로 지정된 mapper 선택
		...
		int blockPage = 2;	// 한 블럭당 출력할 페이지번호의 개수
		// 전체 페이지 수 계산
		int totalPage = (int)Math.ceil((double)totalRecordCount / pageSize);
		// 현재 페이지번호 설정
		/*
			방명록URL?nowPage=		// 이 경우에 페이지번호는 빈값이다.
			방명록URL?nowPage=10	// 10으로 설정
			방명록URL				// null로 판단
		 */
		// 페이지번호가 null이거나 빈값이면 1페이지로 설정
		int nowPage =
			req.getParameter("nowPage")== null || req.getParameter("nowPage").equals("")
			? 1 : Integer.parseInt(req.getParameter("nowPage"));
		
		// 해당 페이지에 출력할 게시물의 구간을 계산
		int start = (nowPage-1) * pageSize + 1;
		int end = nowPage * pageSize;
		
		/*
			서비스 역할의 인터페이스의 추상메서드를 호출하면 mapper가 동작한다.
			전달된 파라미터는	#{param1}, #{param2}	와 같이 순서대로 사용한다.
		 */
	...
	public String listSearch(Model model, HttpServletRequest req) {
		...
		// 위에서 계산한 start, end(게시물의 구간)를 DTO에 저장
		parameterDTO.setStart(start);
		parameterDTO.setStart(end);
	...
	/*
			매핑된 메서드 내에서 session내장객체를 사용하기 위해
			매개변수로 선언해준다.
	 */
	public String write(Model model, HttpSession session, HttpServletRequest req) {
		// session영역에 해당 속성이 없으면 로그아웃 상태이므로 로그인 페이지로 이동
		if(session.getAttribute("siteUserInfo")==null) {
			/*
				현재상태는 글쓰기를 위해 버튼을 클릭했으므로
				로그인이 완료되면 글쓰기 페이지로 이동하는 것이 좋다.
	...
	public ModelAndView loginAction(HttpServletRequest req, HttpSession session) {
		...
		/*
			글쓰기 페이지로의 진입에 실패하면
			로그인 후에 backUrl을 통해 글쓰기 페이지로 이동시킨다.
		 */
	...
	public String modify(Model model, HttpServletRequest req, HttpSession session) {
		...
		/**
		[파라미터를 전달하는 4번째 방법]
			: DTO(혹은 VO)객체에 파라미터를 저장한 후 Mapper로 전달한다.
		 */
		ParameterDTO parameterDTO = new ParameterDTO();
		// 일련번호 저장
	...
	public String modifyAction(HttpSession session, MyBoardDTO myBoardDTO) {
		// 수정페이지에서 전송된 폼값을 커맨드객체를 통해 한꺼번에 받는다.
		// 수정처리 전 로그인 체크
		if(session.getAttribute("siteUserInfo")==null) {
			return "redirect:login.do";
		}
